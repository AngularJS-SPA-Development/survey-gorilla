...
(function() {
  ...
  function GroupListCtrl($scope, $window, groupSvc) {
    ...
    // group 삭제.  DELETE 메소드 /api/v1/groups/:id 요청
  ...
  function GroupViewCtrl($scope, $stateParams, groupSvc) {
    // 1개 group 받기. GET 메소드 /api/v1/groups/:id 요청
    groupSvc.getGroup($stateParams.id).then(function(group) {
      $scope.group = group;
    }); 
  }
  ...
  function GroupEditCtrl($scope, $state, $stateParams, groupSvc) {
    // group 갱신. PUT 메소드 /api/v1/groups/:id 요청
